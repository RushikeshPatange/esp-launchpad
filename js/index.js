...
// Merging two function of same functionality -> SendSerialLine() listenToPort() apeendToTerminal()

async function sendSerialLine(device,writer,reader,term) {
  let dataToSend;
  let textEncoder = new TextEncoder();
  if (!device.writable.locked) writer = device.writable.getWriter();

  if(device === device1){
    // console.log("inside device 1 writer")
    // console.log("Device 1 readable locked: ",device.readable.locked)
    dataToSend = commanddefault.value;
    devicelinehistory.unshift(dataToSend);
    devicehistoryIndex = -1; // No history entry selected
    if (carriageReturndefault.checked == true) dataToSend = dataToSend + "\r";
    if (addLinedefault.checked == true) dataToSend = dataToSend + "\n";
    if (echoOndefault.checked == true) await appendToTerminal(term,"> " + dataToSend);
   commanddefault.value = "";
  }else{
  // console.log("inside device 2 writer")
  // console.log("Device 2 readable locked: ",device.readable.locked)
  dataToSend = command.value;
  lineHistory.unshift(dataToSend);
  historyIndex = -1; // No history entry selected
  if (carriageReturn.checked == true) dataToSend = dataToSend + "\r";
  if (addLine.checked == true) dataToSend = dataToSend + "\n";
  if (echoOn.checked == true) await appendToTerminal(term,"> " + dataToSend);
  command.value = "";
}
...
writer.releaseLock();
  ...
  await listenToPort(device,reader,term);
}

async function listenToPort(device,reader,term) {
  console.log(device,reader,term)
  console.log(device.readable)
  try {
    while (device.readable) {
      ...
      try {
        // while (true) {
          ...
          if (value) {
            let deocdedvalue = new TextDecoder().decode(value);
...
function scrollHistory(direction) {
  ...
  devicehistoryIndex = Math.max(
    Math.min(devicehistoryIndex + direction, devicelinehistory.length - 1),
    -1
  );
  if (devicehistoryIndex >= 0) {
    commanddefault.value = devicelinehistory[devicehistoryIndex];
  } else {
    commanddefault.value = "";
  }
  ...
  if (historyIndex >= 0) {
    document.getElementById("command").value = lineHistory[historyIndex];
  } else {
...
command.addEventListener("keyup", async function (event) {
  if (event.code === "Enter") {
    await sendSerialLine(device2,writer2,reader2,term2)
...
commanddefault.addEventListener("keyup", async function (event) {
  if (event.code === "Enter") {
    await sendSerialLine(device1,writer1,reader1,term1);
  } else if (event.code === "ArrowUp") {
    // Key up
    scrollHistory(1);
  } else if (event.code === "ArrowDown") {
...
disconnectButton1.onclick = async () => {
  if (transport1) {
    // console.log(reader1,writer1,device1)
...
let IntervalForFlashThroughProgramButton = () => {
  let interval = setInterval(() => {
    console.warn("called setInterval after 3 sec");
	console.log(esploader,esploader.status)
    if (esploader !== undefined)
      if (esploader.status === "complete") {
        console.warn("Inside setInetrval completed status");
        postProgramFlashDone();
...
flashButton.onclick = async () => {
  ...
  console.log("After downloadAndFlash")
